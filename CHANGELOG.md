...
- Added `LOCAL_READY`, `REMOTE_HEALTHY`, `REMOTE_PARTIAL`, `REMOTE_DEGRADED`, `PUBLICATION_BLOCKED`, and `BLOCKED` status semantics.
- Added observed-failure classification for capability mismatch, policy/approval gates, remote movement, rate limits, 5xx, DNS/network failures, and other hosted failures without echoing raw error text.
...
- Added repository-aware Connection Advisor recommendations in the First Run Wizard for GitHub, Vercel, Supabase, Netlify, and OpenAI Platform. Recommendations never imply that a connection is installed or authorized; the user connects providers explicitly and the agent verifies actual capabilities.
- Added English/Korean connection guides with least-privilege and user-controlled authentication rules.
