...
- `REMOTE_PARTIAL` — the forge is reachable, but the current authenticated identity, repository policy, approval gate, or operation-specific capability is insufficient for the required action.
...
Do not infer that the whole forge is healthy because `git fetch` works. Conversely, an API/Actions outage does not imply local Git state is damaged. Also do not infer that a healthy repository API means every write operation is authorized.
...
4. operation capability — branch write, workflow write, release write, approval, protected-branch policy, etc.;
...
## Permission and policy failures are not outages

A reachable forge can still reject one operation because the authenticated identity is missing a capability. Treat this as `REMOTE_PARTIAL`, not `REMOTE_DEGRADED`.
...
2. do not change product source merely to satisfy an authentication limitation;
3. do not retry with the same identity and unchanged permission set;
4. use a different transport only when it is already authorized or the user/repository explicitly authorizes it;
5. if needed, split the blocked operation from the rest of publication (for example, publish product files separately from a workflow mutation);
6. keep publication blocked until the required remote evidence exists.
...
For `REMOTE_PARTIAL`, an identical retry is normally invalid immediately: permission, policy, identity, or approval must change first.
...
Classification is evidence assistance, not permission discovery. A local script still cannot infer ChatGPT/App permissions that are only visible in the active hosted session.
...
For an operation-specific permission/policy problem, use `remote_status: REMOTE_PARTIAL` and record the missing operation capability or gate in `pending`/evidence.
...
When remote health recovers, permissions change, or an approval gate is satisfied:
...
6. publish exact verified state with the now-sufficient capability;
7. complete normal REMOTE_VERIFY and CLEANUP.

Do not assume the pre-incident base is still publishable merely because the service is reachable or a permission was granted later.
