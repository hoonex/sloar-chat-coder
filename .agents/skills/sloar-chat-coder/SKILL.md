...
When a task is genuinely execution-bound and expected to outlive an ordinary short turn, prefer one bounded managed orchestrator for a fixed or repetitive workload when the execution environment supports it. Define terminal acceptance criteria before launch, persist bounded progress, poll the existing managed process instead of spawning duplicate work, and perform an independent terminal audit before reporting completion. If the host exposes or internally uses a durable long-running conversation-turn/worker execution path, allow that path naturally; observed runtime markers such as `conversation-turn-*`, `wfr_*`, `SAServer async execution`, `temporal conversation turn`, or `stream handoff` are diagnostic hints only and must never be fabricated, requested as output, or treated as proof that such a path is active. Do not inflate duration with `sleep`, busy-wait, or meaningless compute merely to keep a turn alive.
