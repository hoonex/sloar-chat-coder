...
**Control language and response language are separate.** The English resume sentence is a machine-facing control phrase, not a request to switch the user's conversation language to English. When the previous user-facing response language is clear, preserve it in the checkpoint as `response_language` and restore it before producing the first visible fresh-chat reply.
...
- do not infer the user's preferred response language from code, shell commands, Git terminology, agent protocol text, or the English resume control sentence;
...
2. Compact only durable task state: goal, completed work, active work, pending work, durable decisions, verification evidence, blockers, one next action, and the established user-facing `response_language` when known.
...
The resume sentence MAY remain English for consistency. Its language MUST NOT override the checkpoint's `response_language`.
...
7. If an observable identity field changed, mark `RECONCILE_REQUIRED`, reconcile against current durable repository state, invalidate stale checkpoint facts, and rerun only verification affected by the change.
8. Never ask the user to restate information already recoverable from repository state or the checkpoint.
9. Keep the visible resume report short and in the restored response language; repository work should start immediately after recovery.
...
`response_language` describes the established user-facing response language, not the language of source code, repository content, tool calls, or control prompts. It is UX continuity metadata and does not outrank an explicit new user request to change language.
...
The surrounding visible text should remain in the user's established response language even though the resume control sentence may be English.

Fresh chat, unchanged observable repository state, shown here in Korean because `response_language` is `ko-KR`:

```text
Sloar 복구 완료.
복구한 작업: <goal>
현재 상태: <short stage/status>
다음 작업: <next action>

이어서 진행한다.
```

If working-tree state is unavailable, append a compact qualification such as `Working tree: unobserved` or a localized equivalent rather than implying it is clean.

Fresh chat, repository moved:

```text
Sloar 복구는 완료했지만 handoff 이후 저장소 상태가 변경됐다.
Checkpoint: <old head>
Current: <new head>
현재 저장소 상태를 우선해 reconcile한 뒤 이어서 진행한다.
```
